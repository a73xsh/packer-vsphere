...
variable "iso_checksum" {
  type    = string
  description = "The SHA-512 checkcum of the ISO image EN locale. (e.g. Result of 'shasum -a 512 iso-windows-server-2019.iso')"
  default = ""
}
...
variable "install_mssql2016" {
  type    = list(string)
  description = "Install MSSQL2016"
  default = []
}
...
source "vsphere-iso" "windows-server-datacenter-dexp-mssql2016" {
  vcenter_server       = var.vcenter_server
  username             = var.vcenter_username
  password             = var.vcenter_password
  datacenter           = var.vcenter_datacenter
  ...
  vm_version           = var.vm_version
  notes                = "Built by QA Config Team Packer \n __date: ${local.buildtime}."
  vm_name              = "${ var.vm_guest_os_family }-${ var.vm_guest_os_member }-${ var.vm_guest_os_version }-${ var.vm_guest_os_edition_dc }-en-mssql2016"
  firmware             = var.vm_firmware
  CPUs                 = var.vm_cpu_sockets
  cpu_cores            = var.vm_cpu_cores
  CPU_hot_plug         = false
  ...
  export {
    force = true
    output_directory = "../../../template/${ var.vm_guest_os_family }-${ var.vm_guest_os_member }-${ var.vm_guest_os_version }-${ var.vm_guest_os_edition_dc }-en-mssql2016"
  }
}
  ...
  build {
    ...
    /*
    Uses the File Provisioner to copy the certificate for the Root Certificate Authority.
    ...
    */ 
    ...
    provisioner "powershell" {
      only = [
        ...
        "vsphere-iso.windows-server-datacenter-dexp-mssql2016"
        ]
