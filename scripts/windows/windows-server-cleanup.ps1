...
Import-Certificate -FilePath C:\windows\temp\RootCA.cer -CertStoreLocation 'Cert:\LocalMachine\Root'
Import-Certificate -FilePath C:\windows\temp\SPB.cer -CertStoreLocation 'Cert:\LocalMachine\Root'
Remove-Item C:\windows\temp\RootCA.cer -Confirm:$false
Remove-Item C:\windows\temp\SPB.cer -Confirm:$false
...
New-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.1\Server" -Name "DisabledByDefault" -Value 1 | Out-Null
 #>
# Disable Password Expiration for the Administrator Accounts - (Administration and Ansible)
